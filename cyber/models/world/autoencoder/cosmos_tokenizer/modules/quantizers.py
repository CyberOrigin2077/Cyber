...
from cyber.models.world.autoencoder.cosmos_tokenizer.modules.utils import (
    default,
    entropy,
    pack_one,
    rearrange,
...
class ResidualFSQuantizer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        indices_stack = []
        residual = x
        quantized_out = torch.tensor(0.0, device=x.device, dtype=self.dtype)  # ! modified 0 to torch tensor
        loss_out = torch.tensor(0.0, device=x.device, dtype=self.dtype)  # ! modified 0 to torch tensor
    ...
    def indices_to_codes(self, indices_stack: torch.Tensor):
        quantized_out = 0
        for layer, indices in zip(self.layers, indices_stack.transpose(0, 1), strict=False):
...
class FSQuantizer(nn.Module):
    ...
    def indices_to_codes(self, indices: torch.Tensor, project_out=True) -> torch.Tensor:
        ...
        # ! modified int transformation
        keep_dim_offset = 1 if self.keep_num_codebooks_dim else 0
        is_img_or_video = indices.ndim >= (3 + keep_dim_offset)
    ...
    def forward(self, z: torch.Tensor) -> tuple[torch.Tensor, Any, torch.Tensor]:
        """
        einstein notation
        b - batch
    ...
    def __init__(
        ...
        **ignore_kwargs,
        ...
        if self.remap is not None:
            ...
            logging.info(f"Remapping {self.n_e} indices to {self.re_embed} indices. " f"Using {self.unknown_index} for unknown indices.")
        else:
