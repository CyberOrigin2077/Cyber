...
def load_model(
    ...
    if tokenizer_config is None:
        ...
        full_model, ckpts = _load_pytorch_model(jit_filepath=str(jit_filepath), tokenizer_config=tokenizer_config, device=str(device))
...
def load_encoder_model(
    ...
    """
    if tokenizer_config is None:
        return load_jit_model(jit_filepath, str(device))
    else:
        full_model, ckpts = _load_pytorch_model(jit_filepath=str(jit_filepath), tokenizer_config=tokenizer_config, device=str(device))
...
def load_decoder_model(
    ...
    if tokenizer_config is None:
        ...
        return decoder_model.to(device)


def _load_pytorch_model(jit_filepath: str, tokenizer_config: dict[str, Any], device: str = "cuda") -> tuple[torch.nn.Module, torch.jit.ScriptModule]:
    """Loads a torch.nn.Module from a filepath.
    ...
    model: torch.nn.Module = TokenizerModels[tokenizer_name].value(**tokenizer_config)
    ckpts: torch.jit.ScriptModule = torch.jit.load(jit_filepath)
    return model, ckpts
...
def read_video(filepath: str) -> np.ndarray:
    ...
    if video.ndim == 3:
        video = np.stack([video] * 3, axis=-1)  # type: ignore
    # convert RGBA to RGB
    if video.shape[-1] == 4:
        video = video[..., :3]  # type: ignore
    return video
...
def resize_video(video: np.ndarray, short_size: Optional[int] = None) -> np.ndarray:
...
def numpy2tensor(
    ...
    image = input_image.transpose((0,) + tuple(indices)) / _UINT8_MAX_F  # noqa: RUF005
    if range_min == -1:
        image = 2.0 * image - 1.0
...
def tensor2numpy(input_tensor: torch.Tensor, range_min: int = -1) -> np.ndarray:
    ...
    output_image = output_image.transpose((0,) + tuple(range(2, ndim)) + (1,))  # noqa: RUF005
