...
def export_to_mp4(frames: list, output_mp4_path: str, fps: int):
    ...
    logger.info(f"Exporting {len(frames)} frames of shape: {frames[0].shape}")
    ...
    try:
        ...
        logger.info(f"Video successfully exported to {output_mp4_path}")
    except Exception as e:
        logger.error(f"Error exporting to MP4: {e}")
...
def decode_tokens_to_video(tokens, ckpt_path, output_path, fps, batch_size=16, max_images=None):
    ...
    def decode_latents(video_data):
        ...
        for shard_ind in tqdm(range(math.ceil(len(video_data) / batch_size)), desc="Decoding frames"):
            ...
            # else:
            quant = model.quantize.get_codebook_entry(rearrange(batch, "b h w -> b (h w)"), bhwc=batch.shape + (model.quantize.codebook_dim,)).flip(1)  # noqa: RUF005
            decoded_imgs.append(rescale_magvit_output(model.decode(quant.to(device=device, dtype=dtype))))
    ...
    logger.info(f"Decoded video saved to {output_path}")
